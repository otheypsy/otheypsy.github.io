...
import { createWildWestGame } from '../../game/wildWest/WildWestGame.class'
import WildWestCommand from '../../game/wildWest/WildWestCommand.class'
import { Modal } from '../../components/Modal/Modal.component'

import type { ModalHandler } from '../../components/Modal/Modal.component'
...
const Game: React.FC = memo(() => {
    ...
    const gameContainer: React.RefObject<HTMLDivElement | null> = useRef(null)
    ...
    useEffect(() => {
        const interactHandler = (name: string): void => {
            modalRef.current?.openModal(npcMapping[name])
        }
        const gameInit = async () => {
            if (!gameContainer.current) return
            const game = await createWildWestGame(gameContainer.current, interactHandler)
        ...
        gameInit().then().catch(console.error)
            ...
            <nav className="navbar navbar-expand-lg">
                <div className="container-fluid">
                    <button
                        ...
                        data-bs-target="#gamebar"
                        aria-controls="gamebar"
                        aria-expanded="false"
                        aria-label="Toggle Game Bar"
                    >
                        ...
                        <ul className="navbar-nav me-auto mb-2">
                            <li className="nav-item">
                                <div className="input-group p-2">
                                    <label className="input-group-text" htmlFor="scale">
                                        Scale
                                    </label>
                                    <input
                                        type="number"
