...
class SpeechBubble {
    ...
    drawUp = (renderer: BaseRenderer, text: string, xPix: number, yPix: number, width = 300): void => {
        renderer.saveContext()
        renderer.setScale(1 / this.#config.getScale())
        renderer.configureStyle(style.text)
        const wrapped = renderer.getWrappedText(text, width - this.#padding * 2) as {
            wrappedText: string[]
            height: number
        }
...
const createSpeechBubble = (gameConfig: GameConfig): SpeechBubble => {
    if (!(gameConfig instanceof GameConfig))
        throw new Error('createSpeechBubble:: gameConfig must be an instance of GameConfig')
    return new SpeechBubble(gameConfig)
}

export { createSpeechBubble, SpeechBubble }
